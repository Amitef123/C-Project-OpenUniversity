...
void add_table_item(table *tab, char *key, long value, symbol_type type) {
	...
	new_entry->base = value-(value%16);
	new_entry->offset=value%16;
	new_entry->type = type;
	...
	/* Insert the new table entry and sorted */
	curr_entry = (*tab)->next;
	prev_entry = *tab;
...
void add_value_to_type(table tab, long to_add, symbol_type type) {
	...
	for (curr_entry = tab; curr_entry != NULL; curr_entry = curr_entry->next) {
		if (curr_entry->type == type) {
			...
			curr_entry->base=curr_entry->value -((curr_entry->value)%16);
			curr_entry->offset=((curr_entry->value)%16);
...
table_entry *find_by_types(table tab, char *key, int symbol_count, ...) {
	...
	/* if type is valid and same key, return the entry. */
	do {
		for (i = 0; i < symbol_count; i++) {
