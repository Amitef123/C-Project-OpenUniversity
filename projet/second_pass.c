...
bool process_line_spass(line_info line, long *ic, machine_word **code_img, table *symbol_table) {
	...
	/* Empty or Comment line - no processing needed! */
...
int process_spass_operand(line_info line, long *curr_ic, long *ic, char *operand, machine_word **code_img,
	...
	machine_word *word_to_write_base;
	machine_word *word_to_write_offset;
	...
	if (DIRECT_ADDR == addr || INDEX_ADDR == addr) {
		...
		temp[i]=0;
		long data_to_add;
		table_entry *entry = find_by_types(*symbol_table, temp ,3, DATA_SYMBOL, CODE_SYMBOL, EXTERNAL_SYMBOL);
		if (entry == NULL) {
			printf_line_error(line, "The symbol %s not found", temp);
			return FALSE;
		}
		/*found symbol*/
		...
		if (entry->type == EXTERNAL_SYMBOL) {
			add_table_item(symbol_table, temp, (*curr_ic) + 1, EXTERNAL_REFERENCE);
		}
		/*found symbol*/
		word_to_write_base = (machine_word *) malloc_with_check(sizeof(machine_word));
		word_to_write_base->length = 0;
		word_to_write_base->word.data = build_data_word(addr, data_to_add-(data_to_add%16), entry->type == EXTERNAL_SYMBOL);
		code_img[(++(*curr_ic)) - IC_INIT_VALUE] = word_to_write_base;
		word_to_write_offset = (machine_word *) malloc_with_check(sizeof(machine_word));
		word_to_write_offset->length = 0;
		word_to_write_offset->word.data = build_data_word(addr, (data_to_add%16), entry->type == EXTERNAL_SYMBOL);
		code_img[(++(*curr_ic)) - IC_INIT_VALUE] = word_to_write_offset;
	}
	return TRUE;
}
