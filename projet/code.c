...
bool check_for_index_addr(char *name){
	char *temp;
	char temp1[32];
	int i;
	temp = strchr(name,'[');
	...
	if (temp[0]!='[')
		return FALSE;
	if(is_valid_label_name(temp1))
	{
		temp=temp+1;
		if(temp[0] == 'r' && temp[1]!=0 && temp[1] == '1'&&temp[2] <='5'&&temp[2] >='0'&&temp[3] == ']')
		{
			return TRUE;
		}
		return FALSE;
...
addressing_type get_addressing_type(char *operand) {
	...
	/* if first char is 'r', second is number in range 0-15 and third\four is end of string, it's a register */
	if (operand[0] == 'r' && operand[1] >= '0' && (operand[1] <= '9' && operand[2] == '\0')||(operand[1] == '1' && operand[2] <='5'&&operand[2] >='0'&&operand[3] == '\0')) return REGISTER_ADDR;
	...
	else if (check_for_index_addr(operand)) return INDEX_ADDR;
	else return NONE_ADDR;
}


reg get_register_from_index_addr(char *name) {
	char *temp= strchr(name,'[');
	int digit;
	temp=temp+1;
	if((temp[0] == 'r' && temp[1]=='1'&& isdigit(temp[2]) && temp[3] == ']')){
		digit=10+(temp[2]-'0'); 
		if (digit >= 10 && digit <= 15) return digit;
	}
...
bool validate_operand_by_opcode(line_info line, addressing_type first_addressing,
	...
	if (curr_opcode >= MOV_OP && curr_opcode <= LEA_OP) {
		...
		if (curr_opcode == CMP_OP) {
			return validate_op_addr(line, first_addressing, second_addressing, 4, 4, IMMEDIATE_ADDR,DIRECT_ADDR,INDEX_ADDR,REGISTER_ADDR,
			                            IMMEDIATE_ADDR, DIRECT_ADDR,INDEX_ADDR,REGISTER_ADDR);
		} else if (curr_opcode == ADD_OP || curr_opcode == MOV_OP) {
			return validate_op_addr(line, first_addressing, second_addressing, 4, 3, IMMEDIATE_ADDR,DIRECT_ADDR,INDEX_ADDR,REGISTER_ADDR,
...
reg get_register_by_name(char *name) {
	...
	else if((name[0] == 'r' && name[1]=='1'&& isdigit(name[2]) && name[3] == '\0'))
		digit=10+(name[2]-'0'); 
		if (digit >= 0 && digit <= 15) return digit;
	return NONE_REG; /* No match */
}

opcode_word *build_opword(line_info line,opcode curr_opcode) {
	unsigned long mask=1;
	opcode_word *opword = malloc_with_check(sizeof(opcode_word));
	opword->ARE = 4;
	opword->opcode=mask<<((curr_opcode));
	return opword;
}
...
code_word *get_code_word(line_info line, opcode curr_opcode, funct curr_funct, int op_count, char *operands[2]) {
	...
	codeword->ARE = ((1 << 2) &0xFF); /* A is the only one which is 1 when it's an operation. we treat ARE as a single unit so j */
	...
	if (curr_opcode >= MOV_OP && curr_opcode <= LEA_OP) { /* First Group, two operands */
		...
		if (first_addressing== INDEX_ADDR){
			codeword->src_register = get_register_from_index_addr(operands[0]);
		}
		if (second_addressing == REGISTER_ADDR) {
			codeword->dest_register = get_register_by_name(operands[1]);
		}
		if (second_addressing == INDEX_ADDR) {
			codeword->dest_register = get_register_from_index_addr(operands[1]);
		}
	} else if (curr_opcode >= CLR_OP && curr_opcode <= PRN_OP) {
		...
		if (first_addressing== INDEX_ADDR){
			codeword->dest_register = get_register_from_index_addr(operands[0]);
		}
	}
	return codeword;
}
