...
bool is_valid_label_name_length(char *name,int length) {
	/* Check length, first char is alpha and all the others are alphanumeric, and not saved word */
	return name[0] && strlen(name) <= 31 && isalpha(name[0]) && is_alphanumeric_str_length(name + 1,length) &&
	       !is_reserved_word(name);
}
...
bool is_alphanumeric_str_length(char *string,int length) {
	int i;
	/*check for every char in string if it is non alphanumeric char if it is function returns true*/
...
void free_code_image(machine_word **code_image, long fic) {
	...
	for (i = 0; i < fic; i++) {
		...
		if (curr_word != NULL) {
			/* free code/data word/op word */
			if (curr_word->length == 0) {
				free(curr_word->word.opword);
			}
			else if(curr_word->length>1){
